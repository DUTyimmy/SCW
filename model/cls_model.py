...
def CamDense(pretrained=False, cls_num=30, **kwargs):
    ...
    model.classifier = nn.Conv2d(1664, cls_num, 1, bias=False)
    nn.init.kaiming_normal_(model.classifier.weight.data)
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        # self.classifier1 = nn.Conv2d(1664, 20, 1, bias=False)
    ...
    def gap2d(self, x, keepdims=True):
        out = torch.mean(x.view(x.size(0), x.size(1), -1), -1)
        if keepdims:
            out = out.view(out.size(0), out.size(1), 1, 1)
        return out

    def cam_refine(self, cam):
        cam = F.relu(cam)
        cam = F.interpolate(cam, size=256, mode="bilinear", align_corners=False)
    ...
    def forward(self, x):
        ...
        x = self.features.transition1(x).detach()
        ...
        cam = x
        cls = self.gap2d(x)
        cls = self.classifier(cls).squeeze()

        cam = F.conv2d(cam, self.classifier.weight)
        cam = self.cam_refine(cam)
